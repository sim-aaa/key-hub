...
local openKeyBtn = button(keyScreen, {
	Position = UDim2.fromOffset(0, 244),
	Size = UDim2.new(1, 0, 0, 36),
	Text = "เปิดเว็บรับ Key",
	TextSize = 13,
	BackgroundColor3 = COLORS.accent2,
})
...
openKeyBtn.MouseButton1Click:Connect(function()
	if setclipboard then
		pcall(setclipboard, KEY_PAGE_URL)
		setStatus("ลิงก์เว็บถูกคัดลอกไปยังคลิปบอร์ด", COLORS.ok)
	else
		setStatus("เปิดเว็บด้วยเบราว์เซอร์: " .. KEY_PAGE_URL, COLORS.muted)
	end
end)
...
print("[" .. HUB_NAME .. "] UI loaded — กรอก Key เพื่อเข้าใช้งาน")
>>>>>>> a02d5c1 (feat(ui): add 'Open Key Page' button to in-game key screen)
